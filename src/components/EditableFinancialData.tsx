...
const EditableFinancialData = ({
  ...
  return <Card className="bg-slate-800/50 border-slate-700 backdrop-blur-sm">
      <div className="p-4">
          ...
          <Popover>
            ...
            <PopoverContent className="w-60 bg-slate-800 border-slate-600">
              <div className="space-y-2">
                ...
                <Input id="increment" type="number" value={increment} onChange={e => setIncrement(Number(e.target.value))} className="bg-slate-700 border-slate-600 text-white" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm" onClick={handleClickOutside}>
          {/* Income Card */}
          <div className="bg-black/30 p-3 rounded border border-slate-600 cursor-pointer hover:bg-black/40 transition-colors relative" onDoubleClick={e => {
          e.stopPropagation();
          handleDoubleClick('INCOME');
            ...
            {editingItem === 'INCOME' && <div className="absolute inset-0 bg-black/80 rounded flex items-center justify-center gap-2">
          ...
          {/* Category Cards */}
          {categories.map(category => <div key={category.name} className="bg-black/30 p-3 rounded border border-slate-600 cursor-pointer hover:bg-black/40 transition-colors relative" onDoubleClick={e => {
          e.stopPropagation();
          handleDoubleClick(category.name);
              ...
              {editingItem === category.name && <div className="absolute inset-0 bg-black/80 rounded flex items-center justify-center gap-2">
                  <Button size="sm" variant="outline" onClick={e => {
              e.stopPropagation();
